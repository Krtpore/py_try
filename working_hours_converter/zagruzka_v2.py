    ...
    for y1 in range(1, 30):           #определяем начало и конец таблицы на каждом листе
        for x1 in range(1, 10):
            if sheetread.cell(row=y1, column=x1).value == "Блок" and blck_cntr == 0:
                ...
                blck_cntr = 1
                print("нашел начало!", x1, y1, sheetread.cell(row=table_starts_from_y, column=table_starts_from_x).value)
            if sheetread.cell(row=y1, column=x1).value == "время работы":
                table_ends_on_x = x1 + 6
                table_ends_on_y = y1
                print("нашел конец!", x1 + 6, y1, sheetread.cell(row=table_ends_on_y, column=table_ends_on_x).value)
    blck_cntr = 0
    for y in range(table_starts_from_y+1, table_ends_on_y+2):
        for x in range(table_starts_from_x+1, table_ends_on_x+2):
